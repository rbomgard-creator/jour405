...
ANSWER HERE: Counties with a higher rate of unemployment seem to have less voter turnout. In Prince George's County, their unemployment rate is around 3.5% and their voter turnout rate is around 66%. It also seems like the percentage of white people in a certain population plays into voter turnout. 95% of Garrett County's population is white and their voter turnout is 77%. Population seems to matter less than the rest of the measurements. Median age, education, income, and racial demographics always matter when it comes to voting.

The scatterplots show a positive correlation for income, median age, higher education, and percentage of white within a population. It also shows a negative correlation between voter turnout and unemployment, and voter turnout and population, which further backs up my statements above.
...
ANSWER HERE: This model predicts turnout very well. The p-value is 0.0000029, which means the data is statistically significant. The multiple r-squared value is 0.8539, which means that 85% of the variation in voter turnout is explained by variables in the model. This is still true with the adjusted r-squared value (0.8023), which means that the model is still strong. This is a useful model because of the factors listed above. If the model wasn't statistically significant, none of this information would matter.
...
ANSWER HERE: Model 2 seems to perform slightly better than the others and is the strongest of the four models. Removing some weaker predictors slightly improved model efficiency, but not by a significant amount. But Model 3 only has three predictors and has the same adjusted r-squared as the full model (0.80), showing that a simpler version of the model may work just as well. Model 4 performs the worst, so removing race and only focusing on socioeconomic factors makes the model less predictive.
...
# Model 4
model4 <- lm(turnout ~ income + pct_bachelors_higher + pct_unemployed + median_age, data = md_turnout)
summary(model4)
...
ANSWER HERE: Why do some counties vote at much higher rates than others in Maryland? And why doesn't population matter as much? Income, age, unemployment, and race demographics can be used to see patterns in counties across the state and how they impact voter turnout. Our tests can be used to show the significance in these predictors when it comes to voting. This can further predict how voting might look at the next presidential election in 2028, or just what percentage of each county will most likely vote. This could potentially be compared to other states to see if Maryland's voter turnout is higher or lower than others and why that may be. Does our state do a better job at marketing and advocating for an upcoming election? Do they do a worse job?