...
ANSWER HERE: Yes because it shows a real indicator of whats useful. It also shows why there is statistical significance in the stats from the NFL's 2019 season. This model also has a good prediction factor, which could be helpful for future seasons or more football stats from other leagues.
...
ANSWER HERE: Points Scored has the strongest impact on Wins, showing the highest positive correlation with wins. This indicates that teams that score more points tend to win significantly more games. Yards Gained appears to be the least impactful of the three showing a weaker correlation to wins.

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

I could use this model to guide my game coverage and analysis by focusing on the stats that matter most for winning. Since the results show that scoring points and forcing takeaways are the strongest drivers of wins, my story could center on how and why a team succeeded or failed in those areas. I could also simply use the model to look for these things during a game, even if they don't become the center point of my story. It could just be a good stat or fact to throw in that adds to the significance of a final score or a crazy game.