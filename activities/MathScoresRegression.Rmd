...
**There is a slight or weak positive correlation between FARMS and % Met or Exceeded Math Expectations.**
...
In the middle, you can see the (Intercept) is 59.57897 and the FARMS coefficient is 0.12349. Remember high school algebra? Remember learning the equation of a line? Remember swearing that learning y=mx+b is stupid because you’ll never need it again? Surprise. It’s useful again. In this case, we could try to predict a school's math score by using y=mx+b. In this case, y is the math score, m is 0.12349 and b is 59.57897. So we would multiply a school's FARMS percentage by 0.12349 and then add 59.57897 to it. The result would tell you what the math score would be, according to our model.
...
**This relationship is not statistically significant because the p-value is 0.1225, which is greater than 0.05, which is the significance value. This means the relationship is also weak, which was highlighted in the scatterplot. The adjusted r-square (0.0254) is also very far from 100, which adds reasoning to the insignificance of the correlation. Since neither of these values are significant, coefficient does not matter.**
...
**The largest difference that is positive would be Crooked Billet El Sch, with a projected 64.58 and an actual math score of 95.7. The largeest negative difference is White Marsh El Sch, with a projected 60.99 and an actual math score of 26.1.**
...
**Due to a very weak positive correlation between FARMS percent and scores on a math test, along with tests run to show significance, we cannot prove this statement correct as the connection between the two isn't statistically significant. The idea that socioeconomic status can determine differences in math score is not supported by the data. Some schools with large FARMS percentages how outperformed those with lower ones and vice versa, so it's tough to prove this statement. For a story, this could influence accountability, so I'd want to talk to the board member about why she may feel that way. A story you may also be able to do here would be exploring what truly drives performance differences on standardized tests. Is it learning programs installed into specific schools, instructional quality, how a school is run, accessibility to technology and other learning tools, class size, etc?**
